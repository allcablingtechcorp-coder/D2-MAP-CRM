...
export function LeadDialog({ preset, onClose, onCreated }: { preset?: Partial<LeadInput>; onClose: () => void; onCreated?: (lead: Lead) => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    const result = await workspace.createLead({ companyName, location, ownerName, priority, nextAction, nextActionAt, source: preset?.source ?? "manual" });
    if (!result.ok) {
...
export function ActivityDialog({ onClose, preset }: { onClose: () => void; preset?: { companyName?: string; ownerName?: string } }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!await workspace.addActivity({ kind, subject: subject.trim(), companyName, ownerName, dueAt: new Date(dueAt).toISOString() })) return;
    onClose();
  };
...
export function OpportunityDialog({ onClose, preset }: { onClose: () => void; preset?: { companyName?: string; ownerName?: string } }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!await workspace.addOpportunity({ title: title.trim(), companyName, ownerName, amountCents: Number.isFinite(dollars) && dollars > 0 ? Math.round(dollars * 100) : null, nextAction: nextAction.trim(), expectedCloseAt })) return;
    onClose();
  };
