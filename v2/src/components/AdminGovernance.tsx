...
export function AdminGovernance() {
  const company=useCompany(),emailLabels=useInvitationText();
  ...
  return <>
    ...
    {tab === "invitations" && <Panel title={emailLabels.all} description={emailLabels.listHelp} action={company.superAdmin ? <button className="action-button" onClick={() => setInviteOpen(true)}><MailPlus size={16} />{t("admin.newInvitation")}</button> : undefined}>
      <InvitationList invitations={invitations} canManage={company.superAdmin} repository={runtime.mode === "firebase" ? company.groupMemberships??runtime.memberships : undefined} onRefresh={async()=>{if(runtime.mode!=="firebase")return;const repo=company.groupMemberships??runtime.memberships;setInvitations((await repo.listGovernanceDirectory()).invitations);setAudit((await Promise.all([runtime.memberships.listAudit(),...(company.superAdmin&&company.groupMemberships?[company.groupMemberships.listAudit()]:[])])).flat().sort((a,b)=>b.occurredAt.localeCompare(a.occurredAt)));}}/>
    </Panel>}
...
function InviteDialog({ memberships, invitations, teams: _teams, actor, createRemote, onClose, onCreated }: { memberships: Membership[]; invitations: Invitation[]; teams: Team[]; actor: Membership; createRemote?: (input: { companyIds?:string[]; email: string; role: Exclude<RoleId, "owner">; scope: AccessScope; modules: ModuleId[]; teamIds: string[] }) => Promise<Invitation>; onClose: () => void; onCreated: (invitation: Invitation) => void }) {
  const { t } = useI18n();
  const company=useCompany(),l=useBusinessText(),emailLabels=useInvitationText();
