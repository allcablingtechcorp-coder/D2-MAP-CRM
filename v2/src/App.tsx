...
function Dashboard() {
  ...
  return <>
    <PageHeader eyebrow={t("dashboard.eyebrow")} title={t("dashboard.title")} description={t("dashboard.description")} actions={<ActionButton disabled={!can("lead.create")} onClick={() => setLeadDialogOpen(true)}><Plus size={17} /> {t("dashboard.newLead")}</ActionButton>} />
...
function Leads() {
  ...
  return <>
    <PageHeader eyebrow={t("leads.eyebrow")} title={t("leads.title")} description={t("leads.description")} actions={<ActionButton disabled={!can("lead.create")} onClick={() => setDialogOpen(true)}><UserPlus size={17} /> {t("leads.add")}</ActionButton>} />
...
function Pipeline() {
  ...
  return <>
    <PageHeader eyebrow={t("pipeline.eyebrow")} title={t("pipeline.title")} description={t("pipeline.description")} actions={<ActionButton disabled={!can("opportunity.update")} onClick={() => setDialogOpen(true)}><Plus size={17} /> {t("pipeline.newOpportunity")}</ActionButton>} />
...
function Activities() {
  ...
  return <><PageHeader eyebrow={t("activities.eyebrow")} title={t("activities.title")} description={t("activities.description")} actions={<ActionButton disabled={!can("activity.create")} onClick={() => setDialogOpen(true)}><Plus size={17} /> {t("activities.new")}</ActionButton>} />
...
function Companies() {
  ...
  return <><PageHeader eyebrow={t("companies.eyebrow")} title={t("companies.title")} description={t("companies.description")} actions={<><ActionButton secondary disabled={!can("lead.create") || companies.length === 0} onClick={() => setContactCompanyId(companies[0]?.id ?? null)}><UserPlus size={17} /> {t("companies.newContact")}</ActionButton><ActionButton disabled={!can("lead.create")} onClick={() => setCompanyDialogOpen(true)}><Building2 size={17} /> {t("companies.new")}</ActionButton></>} /><Panel title={t("companies.count", { count: companies.length })} description={t("companies.liveDescription")}><div className="company-grid">{companies.map((company) => { const companyContacts = contacts.filter((contact) => contact.companyId === company.id); const companyDeals = opportunities.filter((item) => item.companyName === company.name); return <article className="company-card" key={company.id}><div className="company-card-top"><span className="company-mark">{company.name.slice(0, 2).toUpperCase()}</span><button className="icon-button" onClick={() => setContactCompanyId(company.id)} aria-label={t("companies.newContact")}><UserPlus size={18} /></button></div><h3>{company.name}</h3><span><MapPin size={13} /> {company.location}</span>{company.industry && <small>{company.industry}</small>}<div className="company-stats"><div><strong>{companyContacts.length}</strong><span>{t("companies.contacts")}</span></div><div><strong>{companyDeals.length}</strong><span>{t("companies.deals")}</span></div></div><div className="company-contact-preview">{companyContacts.slice(0, 2).map((contact) => <span key={contact.id}>{contact.name}{contact.title ? ` · ${contact.title}` : ""}</span>)}</div><button className="company-open" onClick={() => setContactCompanyId(company.id)}><span className="mini-avatar">{company.ownerName.split(" ").map((part) => part[0]).slice(0, 2).join("")}</span>{company.ownerName}<ChevronRight size={16} /></button></article>; })}</div></Panel>{companyDialogOpen && <CompanyDialog onClose={() => setCompanyDialogOpen(false)} />}{contactCompanyId && <ContactDialog companyId={contactCompanyId} onClose={() => setContactCompanyId(null)} />}</>;
}
...
function Reports() {
  ...
  const exportReport = async () => {
    if (!can("report.export")) { setExportMessage(t("workspace.denied")); return; }
    setExporting(true);
    setExportMessage("");
    try {
